function ITEM_dec_recon_SL_SVR(SPM, rad, c, con, meth)
% _
% Decoding from Trials for Reconstruction (searchlight-based SVR)
% FORMAT ITEM_dec_recon_SL_SVR(SPM, rad, c, con, meth)
...
%     meth - a string specifying trial-wise betas to use ('LS-A' or 'LS-S')
% 
% FORMAT ITEM_dec_recon_SL_SVR(SPM, rad, c, con, meth) performs searchlight
...
%  Last edit: 16/07/2024, 13:12 (Vn/a)
...
%-------------------------------------------------------------------------%
suff = meth; suff(3) = '_';
load(strcat(SPM.swd,'/','ITEM_est_1st_lvl_',suff,'/','GLM1.mat'));

% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_SL_SVR: load');
...
%-------------------------------------------------------------------------%
mr_str = strcat(GLM1.swd,'/','mask_SVR.nii');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_SL_SVR: estimate (1)');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_SL_SVR: estimate (2)');
...
for h = 1:s
    ...
    %---------------------------------------------------------------------%
    Yh = G(GLM1.Sess(h).t(1:GLM1.t(h)),:);              % standardize:
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_SL_SVR: estimate (3)');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_SL_SVR: estimate (4)');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_dec_recon_SL_SVR: save');

% Initialize image files
%-------------------------------------------------------------------------%
i = find(c);
H = MA_init_header(SPM, false);
ITEM.swd = strcat(SPM.swd,'/','ITEM_dec_recon_',suff,'/','ITEM_',con,'_SL-',num2str(rad),'mm','/');
...
%         H.descrip = sprintf('ITEM_dec_recon_SL_SVR: out-of-sample correlation coefficient; session %d, target %d', h, i(k));
...
%     H.descrip = sprintf('ITEM_dec_recon_SL_SVR: averaged correlation coefficient; %d sessions, target %d', s, i(k));
...
for k = 1:q
    ...
    H.descrip = sprintf('ITEM_dec_recon_SL_SVR: cross-validated correlation coefficient; %d sessions, target %d', s, i(k));
...
H.descrip = sprintf('ITEM_dec_recon_SL_SVR: voxels per searchlight; %s mm radius', num2str(rad));
