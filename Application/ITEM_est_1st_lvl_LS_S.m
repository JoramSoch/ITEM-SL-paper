function ITEM_est_1st_lvl_LS_S(SPM, mode, nind)
% _
% Estimate First-Level (Scan-Wise) Model (***least squares, separate***)
...
%  Last edit: 19/02/2019, 12:50 (Vn/a)
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_est_1st_lvl_LS_S: load');
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_est_1st_lvl_LS_S: estimate (1)');
...
for h = 1:numel(SPM.Sess)
    ...
    % a model with all trials in one regressor
    GLM1.Sess(h).Ta = blkdiag(ones(GLM1.t(h),1), eye(r));
    GLM1.Sess(h).Xa = GLM1.Sess(h).X * GLM1.Sess(h).Ta;
...
SPM_ReML.xY     = SPM.xY;
SPM_ReML.xM     = SPM.xM;
SPM_ReML.xX.X   = blkdiag(GLM1.Sess.Xa);
if strcmp(mode,'DCT')
    SPM_ReML.xX.K = 1;
end;
if strcmp(mode,'KXY')
...
SPM_ReML.xX.iB  = cumsum(1+GLM1.r);
SPM_ReML.xX.iG  = [];
...
% Init progress bar
%-------------------------------------------------------------------------%
Finter = spm('FigName','ITEM_est_1st_lvl_LS_S: estimate (2)');
...
for h = 1:numel(SPM.Sess)
    ...
    % Estimate each pararmeter separately
    %---------------------------------------------------------------------%
    for k = 1:GLM1.t(h)
        
        % Get design matrix
        %-----------------------------------------------------------------%
        Thk = [zeros(GLM1.t(h),1), ones(GLM1.t(h),1)];
        Thk(k,:) = [1 0];
        Thk = blkdiag(Thk, eye(GLM1.r(h)));
        Xhk = GLM1.Sess(h).X * Thk;
        if strcmp(mode, 'KXY')
            Xhk = spm_filter(GLM1.Sess(h).K, Xhk);
        end;
        
        % Estimate parameters
        %-----------------------------------------------------------------%
        i = GLM1.Sess(h).t(k);
        [Ghk, s2hk] = ITEM_GLM_MLE(Yh, Xhk, Vh, sprintf('Estimate trial-wise response amplitudes for session %d, trial %d',h,k));
        G(i,m_ind)  = Ghk(1,:);
        G(GLM1.Sess(h).t(GLM1.t(h)+1:end),m_ind) = Ghk(3:end,:);
        S2(h,m_ind) = s2hk;
...
for h = 1:numel(SPM.Sess)
    for k = 1:GLM1.tr(h)
        ...
        H.descrip = sprintf('ITEM_est_1st_lvl_LS_S: parameter estimate; session %d, trial %d', h, k);
...
for h = 1:numel(SPM.Sess)
    ...
    H.descrip = sprintf('ITEM_est_1st_lvl_LS_S: variance estimate; session %d', h);
